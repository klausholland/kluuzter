import type { Card } from "@/lib/engine/types";

export function GameCard({
  card,
  faceDown = false,
}: {
  card: Card;
  /** Verdeckte Mystery-Karte: Interpret, Jahr und Titel bleiben verborgen. */
  faceDown?: boolean;
}) {
  if (faceDown) {
    return (
      <div className="flex aspect-[3/4] w-24 shrink-0 flex-col items-center justify-center rounded-xl bg-gradient-to-br from-neutral-700 to-neutral-900 p-2 text-center shadow-lg ring-2 ring-white/10 sm:w-28">
        <p className="text-4xl font-black text-white">?</p>
        <p className="mt-1 text-[10px] uppercase tracking-wide text-white/60">
          Mystery-Song
        </p>
      </div>
    );
  }

  return (
    <div className="flex aspect-[3/4] w-24 shrink-0 flex-col justify-between rounded-xl bg-gradient-to-br from-fuchsia-600 to-indigo-700 p-2 text-center shadow-lg sm:w-28">
      <p className="truncate text-[11px] font-semibold text-white/90">
        {card.artist}
      </p>
      <p className="text-3xl font-black text-white">{card.year}</p>
      <div>
        <p className="truncate text-[11px] text-white/90">{card.title}</p>
        {card.yearSource === "spotify" && (
          <p className="text-[9px] text-amber-200">≈ ungenau</p>
        )}
      </div>
    </div>
  );
}
